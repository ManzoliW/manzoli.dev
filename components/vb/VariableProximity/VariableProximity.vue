...
const lockLetterWidths = () => {
  letterElements.value.forEach((letterEl) => {
    const prev = letterEl.style.fontVariationSettings;
    letterEl.style.fontVariationSettings = props.toFontVariationSettings;
    const maxWidth = letterEl.getBoundingClientRect().width;
    letterEl.style.width = `${maxWidth}px`;
    letterEl.style.textAlign = 'center';
    letterEl.style.fontVariationSettings = prev;
  });
};
...
const initializeLetterElements = () => {
  ...
  const apply = () => {
    lockLetterWidths();
    // Reveal the root only after widths are locked, so the layout the user
    // first sees is the post-mount layout — no width-shift flash.
