...
onMounted(() => {
    ...
    gl.canvas.style.mixBlendMode =
        props.mixBlendMode && props.mixBlendMode !== "none"
            ? props.mixBlendMode
            : "";
    ...
    const program = new Program(gl, {
        ...
        uniforms: {
            ...
            uAnimType: { value: animTypeMap[props.animationType ?? "rotate"] },
            uMouse: { value: [0.5, 0.5] as [number, number] },
            uColorCount: { value: 0 },
            uDistort: { value: typeof props.distort === "number" ? props.distort : 0 },
            uOffset: { value: [toPx(props.offset?.x), toPx(props.offset?.y)] as [number, number] },
            uGradient: { value: gradientTex },
            uNoiseAmount: { value: 0.8 },
            uRayCount: { value: Math.max(0, Math.floor(props.rayCount ?? 0)) },
        },
    });
    ...
    // Build the initial gradient texture and other properties
    ...
    if (Array.isArray(props.colors) && props.colors.length > 0) {
        ...
        gradientTex.format = gl.RGBA;
        gradientTex.type = gl.UNSIGNED_BYTE;
    ...
    // Initial resizes to ensure stable layout
    resize();
    setTimeout(resize, 0);
    ...
    const update = (now: number) => {
        ...
        // Force visible for the first 2 seconds to ensure it starts properly
        const forceVisible = now - startTime < 2000;
        const visible =
            forceVisible || (isVisibleRef.value && !document.hidden);
    ...
    onUnmounted(() => {
        ...
        try {
            ...
            if (glCtx && gradTexRef.value?.texture)
                glCtx.deleteTexture(gradTexRef.value.texture);
        } catch (e) {
            void e;
        }
...
watch(
    () => props.mixBlendMode,
    (mode) => {
        const canvas = rendererRef.value?.gl?.canvas as
            | HTMLCanvasElement
            | undefined;
        if (canvas) {
...
watch(
    ...
    () => {
        ...
        program.uniforms.uAnimType.value =
            animTypeMap[props.animationType ?? "rotate"];

        program.uniforms.uDistort.value =
            typeof props.distort === "number" ? props.distort : 0;
