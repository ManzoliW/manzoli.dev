...
const getBounds = (target: HTMLElement, boundsEl: HTMLElement) => {
  const boundsRect = boundsEl.getBoundingClientRect();
  const targetRect = target.getBoundingClientRect();

  let minX = 0;
  let maxX = boundsRect.width - targetRect.width;
  if (maxX < 0) {
    minX = maxX;
    maxX = 0;
  }
  ...
  let minY = 0;
  let maxY = boundsRect.height - targetRect.height;
  if (maxY < 0) {
    minY = maxY;
    maxY = 0;
  }
...
const setup = () => {
  ...
  const draggable = Draggable.create(target, {
    type: 'x,y',
    bounds: getBounds(target, boundsEl),
    inertia: true,
    zIndexBoost: false,
  ...
  const handleResize = () => {
    if (draggableInstanceRef.value) {
      ...
      const dynamicBounds = getBounds(target, boundsEl);
      draggableInstanceRef.value.vars.bounds = dynamicBounds;
      draggableInstanceRef.value.update();

      const newX = Math.max(dynamicBounds.minX, Math.min(currentX, dynamicBounds.maxX));
      const newY = Math.max(dynamicBounds.minY, Math.min(currentY, dynamicBounds.maxY));
...
watch(
  ...
  () => {
    ...
    const boundsEl = target.parentNode as HTMLElement;
    if (draggableInstanceRef.value && boundsEl) {
      const dynamicBounds = getBounds(target, boundsEl);
      draggableInstanceRef.value.vars.bounds = dynamicBounds;
      draggableInstanceRef.value.update();
    }
...
.sticker-container:focus-visible .sticker-main,
...
.sticker-container:focus-visible .sticker-flap,
